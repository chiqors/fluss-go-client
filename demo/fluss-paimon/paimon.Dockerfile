...
# Make the Fluss lake-storage Paimon plugin jars available on the default Flink classpath.
RUN set -eux; \
    cp -f /opt/flink/paimon/fluss-lake-paimon-*.jar /opt/flink/lib/; \
    cp -f /opt/flink/paimon/hadoop-apache-*.jar /opt/flink/lib/; \
    chown flink:flink /opt/flink/lib/fluss-lake-paimon-*.jar /opt/flink/lib/hadoop-apache-*.jar
