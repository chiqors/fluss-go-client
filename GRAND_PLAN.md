...
- 2026-05-30: completed the first primary-key snapshot batch-scan implementation slice by adding public snapshot storage config, a MinIO-backed remote snapshot downloader, a public `TableClient.SnapshotScanRows(...)` helper, and mock/integration coverage; the first real-cluster validation attempt exposed that the old Pebble/RocksDB local-reader split was too messy for the canonical demo contract and drove the later `rockyardkv` rework.
- 2026-05-30: reworked the exploratory primary-key snapshot batch-scan local reader away from the split Pebble/`gorocksdb` approach onto a single `rockyardkv` read-only path, keeping the public `SnapshotScanRows(...)` slice intact while explicitly leaving real Fluss snapshot compatibility, schema-evolution parity, and production confidence as follow-up validation work.
- 2026-05-30: promoted the current primary-key snapshot batch-scan slice into the canonical Fluss+Paimon support-contract harness by wiring RustFS snapshot storage config into the Go E2E and asserting a real-cluster `SnapshotScanRows(...)` round-trip for the compacted customer table; broader schema-evolution parity and non-demo snapshot portability still remain open.
- 2026-05-30: tightened the Fluss+Paimon demo environment for snapshot coverage by explicitly setting a short `kv.snapshot.interval`; the first E2E attempt showed the server default is 10 minutes, which is too slow for the one-shot support-contract run and caused `GetLatestKvSnapshots` to stay empty within the test timeout.
- 2026-05-30: completed the next snapshot-scan hardening slice by adding a reusable client-level snapshot fetcher abstraction, schema-JSON parsing for historical table schemas, and schema-id-aware row remapping during local snapshot reads; focused integration coverage now proves old-schema snapshot rows can be projected onto the newer requested target schema.
- 2026-05-30: aligned the Go snapshot fetcher direction more closely with the upstream Java client by replacing the hard-coded downloader call with a scheme-based fetcher layer; the built-in implementation now handles `s3://` plus local `file://`/plain-path sources while keeping the public config hook open for additional filesystem backends.
- 2026-05-30: cleaned up the canonical Fluss+Paimon snapshot-scan demo wiring so RustFS/S3 snapshot fetching is configured directly in the Go E2E harness instead of through demo-only environment variables, keeping the SDK defaults local-file-friendly while preserving explicit remote-object-store coverage in the canonical run.
- 2026-05-30: started the primary-key snapshot batch-scan vertical slice by adding upstream-aligned KV snapshot admin metadata support in the Go proto/client layer (`GetLatestKvSnapshots`, `GetKvSnapshotMetadata`, `GetLakeSnapshot`) with local integration coverage; that admin metadata foundation later enabled the current `rockyardkv`-backed snapshot scanner and real-demo support-contract coverage.
...
- 2026-05-30: promoted primary-key typed row helpers into the public `client/` surface with decoded lookup, single-row lookup, and row upsert coverage; this also clarified that true snapshot batch scanning needed the separate Java-aligned snapshot-metadata/file path that is now implemented and exercised in the demo.
